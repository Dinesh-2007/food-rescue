...
export default function ReceiverFoodDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const food = mockDonations.find((d) => d.id === id) || mockDonations[0];
            ...
            <div className="h-72 relative">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                <div className="bg-white/95 dark:bg-black/95 backdrop-blur-md px-3 py-1.5 rounded-xl text-sm font-bold shadow-lg flex items-center gap-1.5 border border-sky-100 dark:border-sky-900">
                  <MapPin className="h-4 w-4 text-sky-500" /> {food.distance} Away
                </div>
                {food.priority === "high" && (
          ...
          <Card className="border-sky-200 dark:border-sky-800 shadow-md relative overflow-hidden">
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <ConfirmDialog
                ...
                trigger={
                  <Button className="w-full rounded-xl gradient-sky text-white border-0 shadow-md h-12 text-base font-bold hover:-translate-y-0.5 transition-transform">
                    Claim Food Now
                  </Button>
                }
              />
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex justify-between items-center border-b pb-2">
                ...
                <span className="font-medium">{new Date(food.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
              </div>
